...
class HandPoseToJointState(Node):

    def __init__(self):
        super().__init__('hand_pose_to_joint_state')
        ...
        self.declare_parameter('robot_config_path', None)
    ...
    def calculate_joint_angles(self, landmarks):
        ...
        for joint in self.joint_config['joints']:
            try:
                ...
                tip_point = np.array([landmarks[joint['landmarks'][1]].x, landmarks[joint['landmarks'][1]].y, landmarks[joint['landmarks'][1]].z])
...
def main(args=None):
    ...
    hand_pose_to_joint_state = HandPoseToJointState()
    try:
        rclpy.spin(hand_pose_to_joint_state)
    except KeyboardInterrupt:
        pass
    finally:
        hand_pose_to_joint_state.destroy_node()
        rclpy.shutdown()
